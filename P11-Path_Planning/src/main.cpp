      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //double car_x = j[1]["x"];
			...
			//Check the number of previous points 
			...
			//Number of previous points are too less	
          	if(prev_size < 2)
          	{
          	...
          	{
          		//If the car is in the same lane as we are 
          		float d = sensor_fusion[i][6];
          		if(d < (2+4*lane+2) && d > (2+4*lane-2) ) // Each lane is 4m wide 
          		{
